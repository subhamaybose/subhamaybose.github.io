...
(function () {
  ...
  init("audience", function () {
    var pills = document.querySelectorAll(".aud button");
    var out = document.getElementById("lede");
    if (!pills.length || !out) return;
    var COPY = {
    ...
    function pick(btn) {
      pills.forEach(function (p) { p.setAttribute("aria-pressed", String(p === btn)); });
      var next = COPY[btn.dataset.aud] || COPY.everyone;
  ...
  init("scroll", function () {
    ...
    if (reduce.matches) {
      ...
      // Kunal-style: hero figure scrubs upward and scales as you leave
      gsap.to("#hero .hero-figure", {
